...
### [2026-06-17] Keyless provider pattern — the single lever is `resolveConnection`

- **Context:** Adding Ollama (local OpenAI-compatible server, no API key) as a provider. The whole app treated "has a saved API key" as a synonym for "provider is usable," which blocked Ollama at every gate.
- **Discovery/Solution:** The cleanest approach is a `keyless?: boolean` flag on `ProviderDef` in `src/providers.ts`. The single hard blocker is `resolveConnection` in `src/config.ts` (line `if (!creds?.api_key) return null`) — once that's conditionalised on `!keyless`, the indirect gates in `StatusBar.tsx`, `App.tsx sendMessage`, and `fetchModelInfo` all fix themselves automatically, since they all check whether `resolveConnection` returned a non-null `ActiveConnection`. The remaining changes are UI-only: suppress the `(no key — run /connect)` marker in the `/models` list, include keyless providers in the add-provider filter, branch the `/connect` step 2 to show a base-URL editor instead of an API key prompt, and fetch installed models live from the running server for the `/models` add flow. The `Authorization: Bearer` header is now conditionally omitted when `api_key` is `""` (Ollama ignores it anyway, but this keeps requests clean). Keyless providers also work with zero `auth.json` entry — `resolveConnection` falls back to `PROVIDERS[id].base_url` when no credentials are saved.
- **Takeaway:** Adding a new OpenAI-compatible provider type in future (keyless or otherwise) requires: (1) add to `PROVIDERS` in `src/providers.ts`, (2) fix `resolveConnection` if the new auth pattern differs, (3) thread any `keyless`-style flag through the 5 UI check-points in `CommandPalette.tsx` (provider list ✓ indicator, add-provider filter, no-key marker, connect step 2, add-model step). `fetchInstalledModels(baseUrl, apiKey)` in `src/openrouter.ts` can be reused for any provider that exposes a standard `/v1/models` endpoint.

---

### [2026-06-17] Reasoning/thinking field variance across providers; Ctrl+M vs Enter in terminals

- **Reasoning field names:** The delta field carrying reasoning tokens differs by provider/model family: `delta.reasoning` (Ollama, OpenRouter), `delta.reasoning_content` (DeepSeek native API), `delta.thinking` (some Anthropic-compatible wrappers). Confirmed via live SSE sniff: Ollama's `gpt-oss:20b` emits `delta.reasoning` with `delta.content === ""` during the thinking phase. The fallback chain `d.reasoning ?? d.reasoning_content ?? d.thinking ?? ""` in `streamCompletion` covers all three. The thinking-phase rule: `isThinking = true` while reasoning chunks arrive and `content` is still `""`. First `delta.content` delta sets `isThinking = false` in the same state update.
- **Ctrl+M vs Enter:** In terminals that don't speak the kitty keyboard protocol (macOS Terminal.app, many tmux setups), Ctrl+M sends byte `0x0D` (`\r`) — physically identical to Enter. opentui's raw-mode parser hits the `s === "\r"` branch first, producing `{ name: "return", ctrl: false }` — the ctrl flag is NOT set and the keys are indistinguishable. Only kitty-protocol terminals (Ghostty, WezTerm, Kitty) emit distinct CSI-u sequences. Safe alternative: avoid the C0 control-code collisions (`\r`, `\t`, `\b`, `\x1b`) — e.g. Ctrl+P (`name: "p", ctrl: true`) works identically in raw and kitty mode.

---

### [2026-06-19] Custom providers persisted in auth.json; opentui `<textarea>` state leaks across same-shape conditional branches without an explicit `key`

- **Context:** Adding user-defined custom OpenAI-compatible providers (`/connect` → `a`/`d`) for local servers (LM Studio, vLLM, etc.), with support for an empty API key.
- **Architecture:** `PROVIDERS`/`PROVIDER_LIST` in `src/providers.ts` are still hardcoded build-time consts (no persistence story of their own). Rather than inventing a second registry file, a custom provider is stored as an **enriched credential** in `auth.json` — `ProviderCredential` gained optional `label`, `keyless`, `custom` fields. `keyless` is derived once at creation time (`api_key.trim() === ""`) and persisted, since `resolveConnection` needs it without re-deriving. `effectiveProviders(auth)` / `effectiveProviderList(auth)` in `providers.ts` merge `{ ...PROVIDERS, ...customFromAuth }` at every call site (`ConnectModal`, `ModelsModal`, `handleConnectSave`, `resolveConnection`) — `auth` was already in scope everywhere, so no prop-drilling was needed. `providers.ts` takes a **type-only** `import type { AuthStore } from "./auth.ts"` to merge with `auth.ts`'s **value** import of `PROVIDERS` — type-only imports are erased at compile time so this isn't a runtime circular dependency.
- **Discovery — textarea content leaks across mutually-exclusive `if` branches:** Built a 3-step add flow (name → URL → key), each step an `if (step === "...") return (<Overlay>...<textarea ref={stepRef} .../>...)`. Live-tested in tmux: typing a name, pressing Enter, then pressing Enter again on the (apparently empty-looking) URL step with **no typing** still advanced past the `if (text)` guard — the URL step's textarea silently contained the previous step's typed text. Root cause: each step returns a structurally identical JSX tree (`Overlay > ModalShell > box > box(column) > text, box(row) > textarea`) from the same component instance. React's reconciler walks by **type + position**, not by which `if` branch executed — since every step has a `<textarea>` at the same tree position with no distinguishing `key`, React patches props (including swapping `ref`) on the *same underlying host node* instead of unmounting/remounting it. opentui's textarea keeps its typed text in an **uncontrolled internal buffer** tied to that host instance, so the buffer survives the "transition" untouched even though the ref object changed.
- **Fix:** Add an explicit, step-distinguishing `key` prop to each textarea (e.g. `key="add-name"`, `key="add-url"`, `key="add-key"`; or `key={`url-${selectedProvider}`}` when the same step re-renders for a different selection). A differing `key` forces React to unmount the old host node and mount a fresh one, clearing the internal buffer. Re-verified live in tmux after the fix: the URL step now renders genuinely empty (placeholder only) and an empty-Enter correctly stays on the step.
- **Takeaway:** Any opentui `<textarea>`/`<select>` rendered from more than one mutually-exclusive branch of the same component (multi-step wizards, mode/step state machines) needs an explicit `key` per branch — otherwise React may silently reuse the host instance and leak its uncontrolled internal state forward. `ModelsModal`'s pre-existing `add-model → add-name` transition has the same shape and is presumed to have the same leak, but it's masked because the leaked text always happens to equal the value already used as that step's placeholder default — left unfixed as out-of-scope/harmless, but worth keying if that code is touched again.
